...
async def cmd_wine(message: types.Message, state: FSMContext):
...
async def cmd_taste(callback: types.CallbackQuery, state: FSMContext):
...
async def mark_chosen_incorrectly(callback: types.CallbackQuery):
    await callback.message.answer(
...
async def cmd_album(callback: types.CallbackQuery, state: FSMContext):
...
async def type_wine_chosen_incorrectly(callback: types.CallbackQuery):
    await callback.message.answer(
